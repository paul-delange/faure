...
#define UNLOCK_STRING @"benjamin smells"
        ...
        if( attr_type == 4 ) {
			...
			if (type == V_ASN1_OCTET_STRING) {
                ...
                if (str_type == V_ASN1_UTF8STRING) {
                    ...
                    if( ![string isEqualToString: @"com.bentley.juste"] )
                        verifyReturnValue = false;
                }
                else {
                    verifyReturnValue = false;
...
bool isUnlockSubscriptionPurchased(void) {
    ...
    NSString* overrideFilePath = [[directories objectAtIndex: 0] stringByAppendingPathComponent: @"override"];
    NSString* override = [NSString stringWithContentsOfFile: overrideFilePath
                                                   encoding: NSUTF8StringEncoding
                                                      error: NULL];
    
    if( [override isEqualToString: UNLOCK_STRING] )
        return YES;
...
void manuallyOverrideSubscriptionLock(void) {
    NSError* error;
    NSArray* directories = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
    NSString* overrideFilePath = [[directories objectAtIndex: 0] stringByAppendingPathComponent: @"override"];
    
    [UNLOCK_STRING writeToFile: overrideFilePath
                         atomically: YES
                           encoding: NSUTF8StringEncoding
                              error: &error];
    DLogError(error);
